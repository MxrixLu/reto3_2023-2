...
def valuesList(rbt, keylo, keyhi, cmp=None):
    try:
        if cmp == None: 
            cmp = rbt['cmpfunction']
        lstvalues = lt.newList('ARRAY_LIST', cmp)
        lstvalues = valuesRangeLists(rbt['root'], keylo, keyhi, lstvalues,
                                rbt['cmpfunction'])
        return lstvalues
    except Exception as exp:
        error.reraise(exp, 'RBT:Values')
...
def valuesMap(rbt, keylo, keyhi, mapa_n, prop): 
    try:
        mapvalues = valuesRangeMap(rbt['root'], keylo, keyhi, mapa_n,
                                rbt['cmpfunction'], prop)
        return mapvalues
    except Exception as exp:
        error.reraise(exp, 'RBT:Values')
        
def valuesBelow(rbt, keyhi, cmpfunction, max):
    try:
        lstvalues = lt.newList('ARRAY_LIST', cmpfunction)
        listVal = valuesBelowkey(rbt['root'], keyhi, lstvalues,
                                cmpfunction, max)
        return listVal
    except Exception as exp:
        error.reraise(exp, 'RBT:Values')
...
def valuesRangeLists(root, keylo, keyhi, lstvalues, cmpfunction):
    try:
        if (root is not None):
            complo = cmpfunction(keylo, root['key'])
            comphi = cmpfunction(keyhi, root['key'])
            ...
            if (complo < 0):
                valuesRangeLists(root['left'], keylo, keyhi, lstvalues,
                            cmpfunction)
            if ((complo < 0) and (comphi >= 0)):
                lstvalues['elements'].extend(root['value']['elements'])
            if (comphi > 0):
                valuesRangeLists(root['right'], keylo, keyhi, lstvalues,
                            cmpfunction)
        lstvalues['size'] = len(lstvalues['elements'])

        return lstvalues
    except Exception as exp:
        error.reraise(exp, 'BST:valuesrange')

def valuesBelowkey(root, keyhi, lstvalues, cmpfunction, max):
    try:
        if (root is not None):
            if max[0]: 
                comphi = cmpfunction(max[1], root['key'], keyhi)
            else:
                comphi = cmpfunction(keyhi, root['key'])

            if (comphi < 0):
                valuesBelowkey(root['left'], keyhi, lstvalues,
                            cmpfunction, max)
                valuesBelowkey(root['right'], keyhi, lstvalues,
                            cmpfunction, max)
            if (comphi > 0):
                lstvalues['elements'].extend(root['value']['elements'])
                valuesBelowkey(root['right'], keyhi, lstvalues,
...
def valuesRangeMap(root, keylo, keyhi, mapvalues, cmpfunction, prop):
    try:
        if (root is not None):
            complo = cmpfunction(keylo, root['key'])
            comphi = cmpfunction(keyhi, root['key'])
            ...
            if (complo < 0):
                valuesRangeMap(root['left'], keylo, keyhi, mapvalues,
                            cmpfunction, prop)
            if ((complo <= 0) and (comphi >= 0)):
                elementos = root['value']['elements']
                if len(elementos) == 1: 
                    if get(mapvalues, elementos[0][prop]) == None:
                        valores = lt.newList('ARRAY_LIST')
                        lt.addLast(valores, elementos[0])
                        put(mapvalues, elementos[0][prop], valores )
                    else: 
                        lt.addLast(get(mapvalues, elementos[0][prop])['value'], elementos[0])
                elif len(elementos) > 1:
                    for elem in elementos: 
                        if get(mapvalues, elem[prop]) == None:
                            valores = lt.newList('ARRAY_LIST')
                            lt.addLast(valores, elem )
                            put(mapvalues, elem[prop], valores )
                        else: 
                            value = get(mapvalues, elem['gap'])['value']
                            lt.addLast(value, elem)
                            put(mapvalues,  elem[prop],  value)
                    
            if (comphi > 0):
                valuesRangeMap(root['right'], keylo, keyhi, mapvalues,
                            cmpfunction, prop)
        return mapvalues
    except Exception as exp:
        error.reraise(exp, 'BST:valuesrange')

def valuesMap1(rbt, keylo, keyhi, mapa_n): 
    try:
        mapvalues = valuesRangeMap1(rbt['root'], keylo, keyhi, mapa_n,
                                rbt['cmpfunction'])
        return mapvalues
    except Exception as exp:
        error.reraise(exp, 'RBT:Values')
        
def valuesRangeMap1(root, keylo, keyhi, mapvalues, cmpfunction):
    try:
        if (root is not None):
            complo = cmpfunction(keylo, root['key'])
            comphi = cmpfunction(keyhi, root['key'])
            ...
            if (complo < 0):
                valuesRangeMap1(root['left'], keylo, keyhi, mapvalues,
                            cmpfunction)
            if ((complo <= 0) and (comphi >= 0)):
                elementos = root['value']['elements']
                if len(elementos) == 1: 
                    if get(mapvalues, int(round(float(elementos[0]['nst'])))) == None:
                        valores = lt.newList('ARRAY_LIST')
                        lt.addLast(valores, elementos[0])
                        put(mapvalues, int(round(float(elementos[0]['nst']))), valores )
                    else: 
                        lt.addLast(get(mapvalues, int(round(float(elementos[0]['nst']))))['value'], elementos[0])
                elif len(elementos) > 1:
                    for elem in elementos: 
                        if get(mapvalues, int(round(float(elem['nst'])))) == None:
                            valores = lt.newList('ARRAY_LIST')
                            lt.addLast(valores, elem )
                            put(mapvalues, elem['nst'], valores )
                        else: 
                            value = get(mapvalues, elem['nst'])['value']
                            lt.addLast(value, elem)
                            put(mapvalues,  elem['nst'],  value)
                    
            if (comphi > 0):
                valuesRangeMap1(root['right'], keylo, keyhi, mapvalues,
                            cmpfunction)
        return mapvalues
    except Exception as exp:
        error.reraise(exp, 'BST:valuesrange')

def valuesAbove(rbt, keyhi, cmpfunction, max):
    try:
        lstvalues = lt.newList('ARRAY_LIST', cmpfunction)
        listVal = valuesBeHighkey(rbt['root'], keyhi, lstvalues,
                                cmpfunction, max)
        return listVal
    except Exception as exp:
        error.reraise(exp, 'RBT:Values')
    
def valuesBeHighkey(root, keyhi, lstvalues, cmpfunction, max):
    try:
        if (root is not None):
            if max[0]: 
                comphi = cmpfunction(max[1], root['key'], keyhi)
            ...
            if (comphi > 0):
                valuesBeHighkey(root['left'], keyhi, lstvalues,
                            cmpfunction, max)
                valuesBeHighkey(root['right'], keyhi, lstvalues,
                            cmpfunction, max)
            if (comphi < 0):
                lstvalues['elements'].extend(root['value']['elements'])
                valuesBeHighkey(root['right'], keyhi, lstvalues,
